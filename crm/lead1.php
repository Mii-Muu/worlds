...
{
$search = $_POST['search'];
$query = "SELECT * FROM `lead` WHERE assign='$user' AND CONCAT(`id`,`name`, `phone`, `need`) LIKE '%".$search."%' ";
$sresult = filterTable($query);
} 
else
{
$query = "SELECT * FROM lead WHERE assign='$user'";
$sresult = filterTable($query);
}
...
?>
		...
		<div class="container-fluid">
			...
			<table class="table table-success bg-opacity-75 table-striped">
				...
				<?php
				$i=1;
				$jet=mysqli_query($spot,"SELECT * FROM lead WHERE assign='$user' ORDER BY id DESC");
				...
				<tr>
					...
					<td class="text-center"><button type="button" class="btn btn-sm btn-success rounded-5" onclick="location.href='edit1.php?id=<?php echo $row['id'];?>'">Edit</button>
    					<button type="button" class="btn btn-sm btn-secondary rounded-5" onclick="location.href='delete1.php?id=<?php echo $row['id'];?>'">Delete</button></td>
				</tr>
				<?php 
        			}
      			?>
                                ...
                                <div class="input-group">
                                    <form method="post">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-3">
                                                <input type="text" name="assign" class="border rounded-5 form-control form-control-md bg-light px-4" placeholder="Assign To" value="<?php echo $_SESSION["user"]; ?>" readonly>
                                            </div>
                                            <div class="col-3">
                            ...
                            <div class="col-lg-12">
                                <div class="container-fluid">
									...
									<table class="table table-warning bg-opacity-75 table-striped">
										...
										include "connect.php";
										$i=1;
										$jet=mysqli_query($spot,"SELECT * FROM lead WHERE assign='$user' AND Date(fdate)=CURDATE()");
                            ...
                            <div class="col-lg-12">
                                <div class="container-fluid">
									...
									<table class="table table-info bg-opacity-75 table-striped">
										...
										include "connect.php";
										$i=1;
										$jet=mysqli_query($spot,"SELECT * FROM lead where assign='$user' AND MONTH(sdate) = MONTH(now()) and YEAR(sdate) = YEAR(now());");
   										while($row = mysqli_fetch_array($jet))
   										{ 
   										?>
										<tr>
